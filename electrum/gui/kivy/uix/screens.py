...
from electrum import bitnet, constants
from electrum.transaction import tx_from_any, PartialTxOutput
from electrum.util import (parse_URI, InvalidBitnetURI, TxMinedInfo, maybe_extract_bolt11_invoice,
...
class SendScreen(CScreen, Logger):
    ...
    def set_ln_invoice(self, invoice: str):
        ...
        self.amount = self.app.format_amount_and_units(lnaddr.amount * bitnet.COIN) if lnaddr.amount else ''
        self.payment_request = None
    ...
    def read_invoice(self):
        ...
        if not address:
            self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a Bitnet address or a payment request'))
            return
        if not self.amount:
            ...
            if self.is_lightning:
                ...
                if self.payment_request:
                    ...
                    if not bitnet.is_address(address):
                        self.app.show_error(_('Invalid Bitnet Address') + ':\n' + address)
                        return
